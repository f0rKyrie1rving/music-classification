"""Pilot and resumable MAEST extraction for expanded development data."""
...
from maest_features import DEST, FEATURE_WIDTH, MODEL_NAME, MaestEncoder
...
def runtime_metadata():
    return {
        ...
        "model_sha256": sha256(DEST / MODEL_NAME),
        "versions": {package: version(package) for package in
                     ("essentia-tensorflow", "numpy", "PyYAML", "six")},
    }
...
def pilot():
    ...
    result = {"track_id": rows[0]["track_id"], "audio_file": rows[0]["audio_file"],
              ...
              "platform": platform.platform(), "fixed_frozen_graph": True,
              "test_tracks": 0, **runtime_metadata()}
    np.save(OUT / "pilot.npy", first, allow_pickle=False)
    save_json(OUT / "pilot.json", result)
...
def extract():
    ...
    if FEATURE_META.exists():
        ...
        if (final["feature_sha256"] != sha256(FEATURES) or final["ids"] != ids
                or final["audio_hashes"] != hashes
                or values.shape != (len(rows), FEATURE_WIDTH) or not np.isfinite(values).all()):
            raise ValueError("Completed MAEST feature cache is stale or invalid.")
        print("MAEST feature cache already complete and verified.")
        return
    ...
    for index, row in enumerate(rows[completed:], completed):
        values[index] = encoder.extract(ROOT / row["audio_file"])
        completed = index + 1
...
if __name__ == "__main__":
    ...
    (pilot if args.command == "pilot" else extract)()
