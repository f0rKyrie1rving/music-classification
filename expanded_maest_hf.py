"""Freeze and run the official MTG-UPF MAEST broad-genre comparison."""
...
PROTOCOL = ROOT / "experiments/MAEST_HF_PROTOCOL.md"
FAILED_PROTOCOL = ROOT / "experiments/MAEST_PROTOCOL.md"
IMPLEMENTATION_NOTE = ROOT / "experiments/MAEST_HF_IMPLEMENTATION_NOTE.md"
LOCK = ROOT / "requirements-mert-lock.txt"
RECEIPT = ROOT / "models/mtg-upf-maest-519l/SOURCES.json"
...
def freeze():
    if PLAN.exists():
        raise FileExistsError("MAEST-HF plan already frozen.")
    ...
    plan.update({"candidate": "mtg-upf/discogs-maest-30s-pw-129e-519l",
        "representations": [REPRESENTATION], "protocol_sha256": sha256(PROTOCOL),
        "failed_protocol_sha256": sha256(FAILED_PROTOCOL),
        "implementation_note_sha256": sha256(IMPLEMENTATION_NOTE),
        "base_plan_sha256": sha256(BASE_PLAN), "expanded_manifest_sha256": sha256(MANIFEST),
        "runtime_lock_sha256": sha256(LOCK), "runner_source_sha256": sha256(Path(__file__)),
        "downloader_source_sha256": sha256(ROOT / "prepare_maest_hf.py"),
        "extractor_source_sha256": sha256(ROOT / "maest_hf_features.py"),
        "driver_source_sha256": sha256(ROOT / "extract_expanded_maest_hf.py"),
    ...
    save_json(PLAN, plan)
    print(f"Frozen MAEST-HF plan: {len(plan['training_ids'])} train, "
...
def load_plan():
    ...
    checks = ((PROTOCOL, "protocol_sha256"), (FAILED_PROTOCOL, "failed_protocol_sha256"),
              (IMPLEMENTATION_NOTE, "implementation_note_sha256"),
              (BASE_PLAN, "base_plan_sha256"), (MANIFEST, "expanded_manifest_sha256"),
              (LOCK, "runtime_lock_sha256"), (Path(__file__), "runner_source_sha256"),
              (ROOT / "prepare_maest_hf.py", "downloader_source_sha256"),
              (ROOT / "maest_hf_features.py", "extractor_source_sha256"),
              (ROOT / "extract_expanded_maest_hf.py", "driver_source_sha256"),
...
def load_features(plan, rows):
    meta = json.loads(FEATURE_META.read_text()); ids = [row["track_id"] for row in rows]
...
def run():
    plan = load_plan(); base_plan, rows, y, groups = load_development()
    ...
    values = load_features(plan, rows); n = len(plan["training_ids"])
    ...
    for j, label in enumerate(LABELS):
        ...
        for config in plan["configs"]:
            ...
            if best is None or trial["mean_ap"] > best[0]["mean_ap"]: best = (trial, scores)
    ...
    precision_thresholds = np.array([value for value, _ in precise]); supported = [flag for _, flag in precise]
    ...
    save_json(OUT / "development_metrics.json", report); load_plan(); load_development()
...
if __name__ == "__main__":
    ...
    parser.add_argument("command", choices=("freeze", "run")); args = parser.parse_args()
